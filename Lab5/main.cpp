...
class SuffTree {
   ...
   public:
    ...
    SuffTree(const std::initializer_list<std::string>& ilist)
        : __size(ilist.size()) {
        __root = new __node{-1, -1, {}, nullptr};
        char i = 0;
    ...
    std::vector<std::string> max_com_substr() const noexcept {
        ...
        std::function<void(const __node*, std::string&)> dfs =
            [&result, &maxlen, &dfs, this](const __node* current,
                                           std::string& str) -> void {
            if (current->owners.size() == __size) {
                str.append(std::string{
                    std::next(__texts[current->owners.front()].begin(),
                              current->begin),
                    std::next(__texts[current->owners.front()].begin(),
                              current->end + 1)});
                if (str.size() > maxlen) {
    ...
    void Print() {
        std::function<void(const __node*, int)> print = [this, &print](
                                                            const __node* cur,
                                                            int depth) {
            if (cur != __root) {
                std::cout << std::string(static_cast<size_t>(depth), '\t');
                std::cout << std::string{
                    std::next(__texts[cur->owners.front()].begin(), cur->begin),
                    std::next(__texts[cur->owners.front()].begin(),
                              cur->end + 1)};
   ...
   private:
    ...
    __node* __Ukkonen(char owner_ind) {
        ...
        for (i = 0; i < text.size(); ++i) {
            ++delay;
            __node* prev =
                nullptr;  // используется для создания суффиксной ссылки
            for (int j = 0, delayed = delay; j < delayed; ++j) {
                if (ptr.cur != root && ptr.cur->owners.back() != owner_ind) {
                    ptr.cur->owners.push_back(owner_ind);
                }
                if (ptr.len == 0 &&
                    !ptr.cur->edges.contains(
                        text[i])) {  // просто добавляем новую ветвь
                    ptr.cur->edges[text[i]] = new __node{
                        i, int(text.size() - 1), {}, nullptr, {owner_ind}};
                    --delay;
                    ...
                    if (ptr.cur->reference)
                        ...
                        ptr.cur = root;
                } else if (ptr.len == 0) {  // начинаем движение по одной из
                                            // ветвей указателем
                    ++ptr.len;
                    ptr.edge = text[i];
                    if (prev && ptr.cur != root) {
                    ...
                    if (ind > ptr.cur->edges[ptr.edge]->end) {
                        // индекс вышел за пределы текущей ветви, продвигаемся
                        // дальше вглубь

                        ptr.cur = ptr.cur->edges[ptr.edge];
                        if (ptr.cur != root &&
                            ptr.cur->owners.back() != owner_ind) {
                            ptr.cur->owners.push_back(owner_ind);
                        }
                    ...
                    // подставляем символы первой строки, при которой создана
                    // эта вершина --------
                    if (__texts[ptr.cur->edges[ptr.edge]->owners.front()]
                               [ind] == text[i]) {  // <-l
                        ...
                        auto old = new __node{
                            next->begin, ind - 1, {}, nullptr, next->owners};
                        next->begin = ind;
                        old->edges = edge_map{};
                        old->edges[__texts[old->owners.front()][ind]] = next;
                        old->edges[text[i]] = new __node{
                            i, int(text.size() - 1), {}, nullptr, {owner_ind}};
                        ...
                        if (ptr.cur != root) {
                            ...
                            ptr.edge =
                                __texts[ptr.cur->edges[ptr.edge]
                                            ->owners.front()]
